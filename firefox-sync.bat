...
REM Finde das Default-Profil (*.default-release oder erstes Profil)
set "PROFILE_DIR="
...
echo Nach: %BACKUP_DIR%

if not exist "%BACKUP_DIR%" mkdir "%BACKUP_DIR%"

REM Nur relevante Daten kopieren (kein Cache, kein Crash-Reports)
REM Extensions, Einstellungen, Bookmarks, Cookies, Storage
robocopy "%PROFILE_DIR%" "%BACKUP_DIR%" /mir /xd cache2 startupCache thumbnails crashes saved-telemetry-pings datareporting /xf parent.lock .parentlock >nul 2>&1
